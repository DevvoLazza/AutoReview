...
export function LocationPreferences({
  ...
  location: { id: string; defaultLanguage: string; tone: string };
  disabled: boolean;
  save: (value: { defaultLanguage: string; tone: string }) => void;
}) {
  ...
  return (
    <form
      ...
      onSubmit={(event) => {
        ...
        save({ defaultLanguage: language, tone });
      }}
    >
      <label>
        Lingua della sede
